...
const userRules = {
  sharePerson: [{ required: true, message: "请选择用户", trigger: "change" }],
};
...
const handleRenameBlur = (file) => {
  if (renameFile.oldName !== renameFile.fileName) {
    fileStore.renameFileAction(
      { fileId: file.fileId, newName: renameFile.fileName },
      directory[defaultActive.value - 1].label
    );
  }
  renameFile.fileId = "";
  ...
  switch (action) {
    ...
    case "view":
      if (contextMenu.file.type === "folder") {
        ...
        fileStore.fetchFileList({
          type: directory[defaultActive.value - 1].label,
          nowPath: contextMenu.file.fileId,
        });
        return;
      }
