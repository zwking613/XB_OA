    ...
    <el-row :justify="formParameter.isSpan ? 'end':'start'">
      <el-col v-for="item in formParameter.fields" :key="item.label+item.prop"
        ...
        <el-form-item :offset="0" :label="item.label" :prop="item.prop" :label-width="formParameter.formItemLabelWidth">
          <component v-bind="{ ...item }" v-if="item.component && !item.check" :is="item.component" :item="item"
            :formParameter="formParameter" v-model="formParameter.form[item.prop]" />
          <!-- input -->
          <el-input v-bind="{ ...item}" v-if="item.type==='input'&& !item.check" :style="item.style" type="text"
            autocomplete="off" :placeholder="`请输入${item.label}`" v-model="formParameter.form[item.prop]"
            @change="item.onChange" />
          <!-- input数字输入框 -->
          <el-input  v-bind="{ ...item}" v-if="item.type==='inputNumber'&& !item.check" type="number" :style="item.style" autocomplete="off"
            :placeholder="`请输入${item.label}`" v-model="formParameter.form[item.prop]" @change="item.onChange" />
          <!-- input只读框 -->
          <el-input  v-bind="{ ...item}" v-if="item.type==='inputReadOnly'&& !item.check" type="text" :style="item.style" autocomplete="off"
            :placeholder="`请输入${item.label}`" disabled v-model="formParameter.form[item.prop]" />
          <!-- 文本域 -->
          <el-input  v-bind="{ ...item}" v-if="item.type==='textarea'&& !item.check" type="textarea" :style="item.style" autocomplete="off"
            :placeholder="`请输入${item.label}`" v-model="formParameter.form[item.prop]" @change="item.onChange" />
          <!-- select下拉框 -->
          <el-select  v-bind="{ ...item}" v-if="item.type==='select'&& !item.check" v-model="formParameter.form[item.prop]" filterable
            :placeholder="`请选择${item.label}`" @change="item.onChange" fit-input-width :multiple="item.multiple"
            style="width:100%">
            <el-option v-for="i in item.options" :key="i.value" :label="i.label" :value="i.value" />
          </el-select>
          <!-- select-tree -->
          <el-tree-select  v-bind="{ ...item}" v-if="item.type==='selectTree'&& !item.check" v-model="formParameter.form[item.prop]"
            :data="item.options" :multiple="item.multiple" :render-after-expand="false"
            :show-checkbox="item.showCheckbox" style="width:100%" />
          <!-- switch -->
          <el-switch  v-bind="{ ...item}" v-if="item.type==='switch'&& !item.check" v-model="formParameter.form[item.prop]"
            :disabled="item.disabled" />
          <!-- radio -->
          <el-radio-group  v-bind="{ ...item}" v-if="item.type==='radio'&& !item.check" v-model="formParameter.form[item.prop]"
          ...
          <!-- checkbox -->
          <el-checkbox-group  v-bind="{ ...item}" v-if="item.type==='checkbox'&& !item.check" v-model="formParameter.form[item.prop]"
            @change="item.onChange">
            <el-checkbox v-for="i in item.options" :key="i.label" :label="i.label" :name="i.title"
              :disabled="item.disabled" />
          </el-checkbox-group>
          <!-- colorPicker -->
          <el-color-picker  v-bind="{ ...item}" v-if="item.type==='colorPicker'&& !item.check" v-model="formParameter.form[item.prop]"
            @change="item.onChange" :disabled="item.disabled" :show-alpha="item.showAlpha"
            :predefine="item.predefineColors" />
          <!-- date-->
          <el-date-picker  v-bind="{ ...item}" v-if="item.type==='datePick'&& !item.check" v-model="formParameter.form[item.prop]"
          ...
          <!-- Cascader 级联选择器 -->
          <!-- Rate 评分 -->
          <el-rate  v-bind="{ ...item}" v-if="item.type==='rate'&& !item.check" v-model="formParameter.form[item.prop]"
            :disabled="item.disabled" @change="item.onChange" :allow-half="item.allowHalf" />
          <!-- Slider 滑块 -->
          <el-slider  v-bind="{ ...item}" v-if="item.type==='slider'&& !item.check" v-model="formParameter.form[item.prop]"
